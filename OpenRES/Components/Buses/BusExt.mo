within OpenRES.Electrical.Buses;
model BusExt
  ...
  parameter Integer nn(min=0) = 0 "Number of right connections"
    annotation (Dialog(connectorSizing=true), HideResult=true);
  OpenRES.Interfaces.PwPin p[np] annotation (Placement(
      visible=true,
      transformation(
        origin={-20.0001,1},
      ...
      iconTransformation(
        ...
        extent={{-4,-60},{4,60}},
        rotation=0)));
  OpenRES.Interfaces.PwPin n[nn] annotation (Placement(
      visible=true,
      transformation(
        origin={20,0},
...
<td><p><a href=\"mailto:luigiv@kth.se\">luigiv@kth.se</a></p></td>
    ...
    uses(OpenRES(version="1.0.0"), Modelica(version="3.2.1")));
end BusExt;
